...
// every deck-aligned 48-card window was guaranteed exactly 4-of-each-rank,
...
describe("buildShoe -- multi-deck shoe is shuffled as one unit, not deck-by-deck", () => {
  it("does not leave the first 48 cards as a rigid, always-perfectly-balanced block", () => {
    ...
    const isPerfectlyBalanced = (counts: Record<string, number>) =>
      Object.keys(counts).length === 12 && Object.values(counts).every((n) => n === 4);

    // Under the old bug this was true on EVERY trial (probability 1) --
    // deck 1's own shuffle always fully occupied the first 48-card window.
    ...
    for (let trial = 0; trial < 30 && !sawUnbalancedWindow; trial += 1) {
      ...
      const firstWindow = rankCounts(shoe.slice(0, 48));
      if (!isPerfectlyBalanced(firstWindow)) sawUnbalancedWindow = true;
    }
    expect(sawUnbalancedWindow).toBe(true);
  });

  it("still preserves the full shoe's overall composition -- 8 of each rank for a 2-deck shoe", () => {
    const shoe = buildShoe(2);
    expect(shoe).toHaveLength(96);
    const counts: Record<string, number> = {};
    shoe.forEach((c) => {
      counts[c.name] = (counts[c.name] ?? 0) + 1;
    });
