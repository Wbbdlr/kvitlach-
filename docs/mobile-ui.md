...
| T3 | Banker's chip controls overlap other elements | Same cause as T14: everything a player might reach for was collapsed behind one unlabelled `⋯`, so the panel it opened had to hold a wrapped grid of icons | **Fixed** with T14 — the menu is a column of named rows |
...
| T5 | Bank total and reserved/free sit too high; viewer's readout is stranded in a corner; empty band between felt and dock | Every placement decision in this refactor was driven by phone landscape, where the centre column is over-subscribed. On a desktop the problem is **inverted** — the middle is empty and the extremes are far away | **Largely answered by T11**: the readout is now draggable and remembered, so "it should be nearer the middle on my PC" is a preference a player sets rather than a default anyone has to be right about. The bank's own height is unchanged and still open if it still reads wrong |
