...
// The Eleveroon reject used to fly out to the discard pile and unmount
// itself about two seconds after it landed, on the theory that the pile was
// the record and a ring left in the hand was clutter. Reported as a bug and
// corrected: "that card should appear as part of the hand until the next
// round, so people can still see that it happened. That's the point of having
// the overlay, so people can see why the hand didn't bust." An explanation for
// an 11 that plainly should have busted the hand is worth nothing if it is
// gone before anyone looks up.

describe("CardView -- Eleveroon-rejected card", () => {
  it("shows the ring and badge, and plays the reject pop, when freshly dealt this session", () => {
  ...
  it("no longer flies the card out of the hand", () => {
    const { container } = render(<CardView card={rejectedCard} pastFirstPaint />);
    expect(container.querySelector(".k-card-discard-out")).toBeFalsy();
  });

  it("stays in the hand indefinitely rather than unmounting itself", () => {
    vi.useFakeTimers();
    try {
      const { container } = render(<CardView card={rejectedCard} pastFirstPaint />);
      act(() => {
        // Comfortably past the old 1780ms fly-out. The card is still there,
        // and stays there until the hand itself clears at the next round.
  ...
  it("shows the ring to a client that reconnected after the reject, without replaying the pop", () => {
    // This case used to render nothing at all: the card had flown off in an
    // earlier session, so there was nothing left to show. That is exactly the
    // player this fix is for -- they arrive at a hand whose arithmetic looks
    // wrong and need to see why.
    const { container, getByText } = render(<CardView card={rejectedCard} pastFirstPaint={false} />);
    expect(container.querySelector(".k-card-elev")).toBeTruthy();
    expect(getByText("Eleveroon")).toBeInTheDocument();
    expect(container.querySelector(".k-card-elev-in")).toBeFalsy();
  });
